...
before(async () => {
  const mod = await import(
    new URL('../components/A11yPanel/_A11yPanel.js?component-test', import.meta.url)
  );
  ({
    ACCESSIBILITY_OPTIONS,
    BODY_FONT_OPTIONS,
...
function mountPreferences() {
  const element = document.createElement('pix-a11y-panel');
...
describe('PixA11yPanel - rendering & structure', () => {
  test('registers the custom element as pix-a11y-panel', () => {
    const element = mountPreferences();
    assert.equal(customElements.get('pix-a11y-panel'), PixA11yPanel);
    assert.equal(element.tagName.toLowerCase(), 'pix-a11y-panel');
  });
  ...
  test('renders the popover panel with correct role and aria attributes', () => {
    ...
    assert.ok(panel.id.startsWith('a11y-panel-panel-'));
  });
...
describe('PixA11yPanel - document application & persistence', () => {
...
describe('PixA11yPanel - exported API surface', () => {
  test('exports STORAGE_KEY', () => {
    assert.equal(STORAGE_KEY, 'pix-a11y-panel');
  });
...
describe('PixA11yPanel - readPreferences & normalizePreferences', () => {
...
describe('PixA11yPanel - applyPreferencesToDocument', () => {
...
describe('PixA11yPanel - popover behaviour', () => {
...
describe('PixA11yPanel - CSS & component styles', () => {
  test('loads local component styles and registers them autonomously', async () => {
    mountPreferences();

    const componentSource = await readFile(
      new URL('../components/A11yPanel/_A11yPanel.js', import.meta.url),
      'utf8'
    );
    const componentCss = await readFile(
      new URL('../components/A11yPanel/_A11yPanel.css', import.meta.url),
      'utf8'
    );

    assert.ok(document.adoptedStyleSheets.length >= 1);
    assert.ok(
      document.adoptedStyleSheets[0].cssText.includes('pix-a11y-panel [data-preferences-toggle]')
    );
    assert.ok(componentSource.includes("import componentCSS from './_A11yPanel.css?raw';"));
  ...
  test('component CSS includes accent-color-selector integration styles', async () => {
    ...
    assert.ok(componentCss.includes('@layer pix-a11y-panel'));
  });
...
describe('PixA11yPanel - re-initialisation & multiple instances', () => {
...
describe('PixA11yPanel - docs site adaptation', () => {
