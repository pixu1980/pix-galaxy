...
`,
  },
  {
    ...
    lang: 'js',
    code: `import { readPreferences, DEFAULT_PREFERENCES } from '@pix-galaxy/pix-a11y-panel';\n\nconst prefs = readPreferences();\nconsole.log(prefs.fontScale); // '100%'`,
  },
  {
    title: 'Apply programmatically',
    description: 'Apply preferences without mounting the component.',
    lang: 'js',
    code: `import { applyPreferencesToDocument } from '@pix-galaxy/pix-a11y-panel';\n\napplyPreferencesToDocument({\n  fontScale: '125%',\n  increaseContrast: true,\n  radiusPreset: 'square',\n});`,
  },
]);
...
export function createDocsSite({ mount, docs, examples: exampleEntries, meta, afterRender }) {
  ...
  function setupPreferenceWatcher() {
    ...
    const observer = new MutationObserver(() => {
      ...
      const changed = Object.keys(newPrefs).some((key) => newPrefs[key] !== state.docPrefs[key]);
      if (!changed) return;
    ...
    setInterval(() => {
      const root = document.documentElement;
      const snapshot = OBSERVED_PROPERTIES.map((prop) =>
        prop.startsWith('--') ? root.style.getPropertyValue(prop) : root.style[prop]
      ).join('|');
