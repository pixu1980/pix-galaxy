...
`pix-a11y` is a lightweight Web Component that manages display preferences in the browser with zero runtime dependencies.

## Persistence

All preferences are stored in `localStorage` under the key `pix-a11y`. On subsequent page loads, the saved preferences are restored automatically.

```js
// Reading the stored preferences
const raw = localStorage.getItem('pix-a11y');
