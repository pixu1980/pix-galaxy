 ...
 * Manages accent color, accessibility, typography, line-height, and
...
import componentCSS from './_A11y.css?raw';

const STORAGE_KEY = 'pix-a11y';
const ELEMENT_NAME = 'pix-a11y';
...
function findLineHeightOption(optionId) {
  return (
    LINE_HEIGHT_OPTIONS.find((option) => option.id === optionId) ||
    LINE_HEIGHT_OPTIONS.find((option) => option.id === DEFAULT_PREFERENCES.lineHeight)
  );
}
...
function normalizePreferences(preferences = {}) {
  ...
  return {
    ...
    lineHeight: findLineHeightOption(candidate.lineHeight).id,
...
function renderLineHeightOptions(selectedValue) {
...
class PixA11y extends ComponentBase {
  ...
  constructor() {
    ...
    popoverCount += 1;
    this._panelId = `a11y-panel-${popoverCount}`;
    ...
    template.innerHTML = `
      <div data-preferences-shell>
        ...
        >
          ...
          <section data-preferences-group data-preferences-card aria-labelledby="${this._panelId}-accent">
          ...
          <section data-preferences-group data-preferences-card aria-labelledby="${this._panelId}-accessibility">
            <div data-preferences-group-title>
              <h3 id="${this._panelId}-accessibility">Accessibility</h3>
              <p>Reduce friction with calmer motion, solid surfaces, and stronger contrast.</p>
            </div>
            <div data-preferences-card-body data-preferences-checklist>
              ${renderAccessibilityOptions(this._panelId, this.preferences)}
            </div>
          </section>

          <section data-preferences-group data-preferences-card aria-labelledby="${this._panelId}-type">
            <div data-preferences-group-title>
              <h3 id="${this._panelId}-type">Typography</h3>
              <p>Tune scale, line-height, and font stacks for headings, body copy, and code.</p>
            </div>

            <div data-preferences-card-body data-preferences-grid>
              ...
              <div data-preferences-field>
                <label for="${this._panelId}-line-height">Line-height</label>
                <select id="${this._panelId}-line-height" name="lineHeight">
                  ${renderLineHeightOptions(this.preferences.lineHeight)}
                </select>
              </div>
          ...
          <section data-preferences-group data-preferences-card aria-labelledby="${this._panelId}-shape">
            <div data-preferences-group-title>
              <h3 id="${this._panelId}-shape">Corners</h3>
              <p>Switch between square edges, restrained 4px rounding, and soft squircle corners.</p>
            </div>

            <div data-preferences-card-body data-preferences-choice-grid role="radiogroup" aria-labelledby="${this._panelId}-shape">
