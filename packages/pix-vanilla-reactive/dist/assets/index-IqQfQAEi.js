...
@layer pix-galaxy {
  @layer pix-color-scheme-selector {
    ...
    pix-color-scheme-selector label {
      position: relative;
      display: inline-flex;
      align-items: center;
    ...
    pix-color-scheme-selector label:hover {
      background: var(--pix-ds-surface-panel);
    }

    pix-color-scheme-selector label + label {
      border-left: var(--pix-ds--ctrl--border);
    }

    pix-color-scheme-selector label:first-of-type {
      border-start-start-radius: calc(var(--pix-ds--r--md) - 1px);
      border-end-start-radius: calc(var(--pix-ds--r--md) - 1px);
    }

    pix-color-scheme-selector label:last-of-type {
      border-start-end-radius: calc(var(--pix-ds--r--md) - 1px);
      border-end-end-radius: calc(var(--pix-ds--r--md) - 1px);
    }

    pix-color-scheme-selector label[data-active] {
      background: var(--pix-ds-accent-primary);
      color: light-dark(oklch(1 0 0), oklch(0.96 0.01 75));
    }

    pix-color-scheme-selector label[data-active]:hover {
      background: color-mix(in srgb, var(--pix-ds-accent-primary) 88%, black);
    }

    pix-color-scheme-selector svg {
      width: 1.25rem;
      height: 1.25rem;
      display: block;
      pointer-events: none;
    ...
    pix-color-scheme-selector input[type='radio'] {
      position: absolute;
      width: 1px;
      height: 1px;
      padding: var(--pix--s--0);
...
`,Je=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none">
...
`,K=`pix-color-scheme`,q=[`light`,`dark`,`system`],Ye=`pix-color-scheme-selector`,J={light:`light`,dark:`dark`,system:`light dark`};function Xe(){try{return window.localStorage}catch{return null}}var Y=null;function Ze(){return typeof document>`u`||!(`adoptedStyleSheets`in document)||typeof globalThis.CSSStyleSheet!=`function`||typeof globalThis.CSSStyleSheet.prototype.replaceSync!=`function`?null:(Y||(Y=new CSSStyleSheet,Y.replaceSync(Ge)),document.adoptedStyleSheets.includes(Y)||(document.adoptedStyleSheets=[...document.adoptedStyleSheets,Y]),Y)}var X=class extends HTMLElement{static ensureComponentStyles(){return Ze()}constructor(){super(),this._onChange=this._onChange.bind(this),this.currentScheme=this.getInitialScheme()}connectedCallback(){this.constructor.ensureComponentStyles(),this.render(),this.attachEventListeners()}render(){let e=document.createElement(`template`);this.textContent=``,e.innerHTML=`
      <section
    ...
    `,this.appendChild(e.content.cloneNode(!0)),this._inputs=Array.from(this.querySelectorAll(`input[name="color-scheme"]`)),this._options=Array.from(this.querySelectorAll(`label`)),this.applyScheme(this.currentScheme)}attachEventListeners(){this._inputs?.forEach(e=>{e.addEventListener(`change`,this._onChange)})}disconnectedCallback(){this._inputs?.forEach(e=>{e.removeEventListener(`change`,this._onChange)})}getOrCreateMeta(){let e=document.querySelector(`meta[name="color-scheme"]`);if(e)return e;let t=document.createElement(`meta`);return t.setAttribute(`name`,`color-scheme`),document.head.appendChild(t),t}getSavedScheme(){let e=Xe()?.getItem(K);return q.includes(e)?e:null}getSchemeFromMeta(){let e=document.querySelector(`meta[name="color-scheme"]`)?.getAttribute(`content`)||``;return e===`light`?`light`:e===`dark`?`dark`:`system`}getInitialScheme(){return this.getSavedScheme()||this.getSchemeFromMeta()||`system`}updateOptionState(){let e=this.querySelector(`input[value="${this.currentScheme}"]`);e&&(e.checked=!0),this._options?.forEach(e=>{let t=e.querySelector(`input`);e.toggleAttribute(`data-active`,t?.value===this.currentScheme)})}applyScheme(e){let t=q.includes(e)?e:`system`,n=this.getOrCreateMeta(),r=document.documentElement;this.currentScheme=t,Xe()?.setItem(K,t),n.setAttribute(`content`,J[this.currentScheme]),this.currentScheme===`system`?(r.removeAttribute(`data-color-scheme`),r.style.colorScheme=`light dark`):(r.setAttribute(`data-color-scheme`,this.currentScheme),r.style.colorScheme=this.currentScheme),this.updateOptionState()}_onChange(e){let t=e.target.value;this.applyScheme(t)}};X.ensureComponentStyles(),globalThis.customElements?.get(Ye)||globalThis.customElements.define(Ye,X);var Qe=X;(function(){if(typeof document>`u`||typeof CSSStyleSheet!=`function`)return;let e=new CSSStyleSheet;e.replaceSync(n),document.adoptedStyleSheets.includes(e)||(document.adoptedStyleSheets=[...document.adoptedStyleSheets,e])})();function Z(e){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function $e(e,t){return e.match(/^#\s+(.+)$/mu)?.[1]?.trim()||t}function et(e){return e.split(`/`).pop().replace(/\.md$/u,``)}function tt(){let e=new B;return e.code=({text:e,lang:t=``})=>`<pre is="pix-highlighter" tabindex="0" data-lang="${Z(String(t||`text`).trim().toLowerCase()||`text`)}"><code>${Z(e)}</code></pre>`,new Ue({renderer:e,gfm:!0})}function nt(e){let t=tt();return e.map(({markdown:e,sourcePath:n})=>({slug:et(n),title:$e(e,et(n)),html:t.parse(e),sourcePath:n}))}function rt(e){return Z(e)}function it(e,t=`example-card`){return e.map(e=>`
        <article data-part="${Z(t)}">
          <section data-part="example-copy">
            <h3>${Z(e.title)}</h3>
            <p>${Z(e.description)}</p>
          </section>
          <pre is="pix-highlighter" tabindex="0" data-lang="${Z(e.lang)}"><code>${rt(e.code)}</code></pre>
        </article>
      `).join(``)}function at({mount:e,docs:t,examples:n,meta:r,afterRender:i}){let a={activeSlug:t[0]?.slug||``};e.addEventListener(`click`,e=>{let t=e.target.closest(`[data-site-doc]`);if(t){a.activeSlug=t.dataset.siteDoc,o();return}});function o(){let o=t.find(e=>e.slug===a.activeSlug)||t[0],s=t.filter(e=>e.slug!==`examples`).map(e=>`
          <button
            type="button"
...
)}</ul>\`;`}];async function lt(){if(typeof document>`u`)return;let e=document.querySelector(`#app`);if(!e)return;let[{default:t},{default:n},{default:r},{default:i},{default:a},{default:o},s,c,l]=await Promise.all([$(()=>import(`./getting-started-VXprhVVC.js`),[]),$(()=>import(`./how-it-works-CnEy7j_f.js`),[]),$(()=>import(`./api-xCZBw87Z.js`),[]),$(()=>import(`./examples-C7wSr3Gv.js`),[]),$(()=>import(`./releasing-Eo-BKbN3.js`),[]),$(()=>import(`./package-Ch9a2sr0.js`),[]),$(()=>import(`./src-p2WNpP-W.js`),[]),$(()=>import(`./artifact-ChUfeZiO.js`),[]),$(()=>Promise.resolve().then(()=>We),void 0)]),{enhancePixHighlighters:u}=c;at({mount:e,docs:nt([{markdown:t,sourcePath:`src/docs/content/getting-started.md`},{markdown:n,sourcePath:`src/docs/content/how-it-works.md`},{markdown:r,sourcePath:`src/docs/content/api.md`},{markdown:i,sourcePath:`src/docs/content/examples.md`},{markdown:a,sourcePath:`src/docs/content/releasing.md`}]),examples:ct,meta:{version:o.version,componentName:`pix-vanilla-reactive`,componentTag:`pix-vanilla-reactive`,description:"Zero-dependency reactive UI framework: Proxy Store, fine-grained Signals, html`` template engine, and custom elements.",liveHtml:`
        <pix-color-scheme-selector></pix-color-scheme-selector>
        <div style="margin-top:0.5rem;font-size:0.875rem;">Signals · Store · html`` templates</div>
      `},afterRender(e){u(e)}})}lt();