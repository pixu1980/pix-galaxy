...
description: 'Use when building Custom Elements v1, web components, customized built-ins, interactive widgets, reusable HTML/JS components, component scaffolds, component lifecycle code, adoptedStyleSheets styling, componentDecorator wiring, static attributes, static events, or pix-galaxy shared component runtime helpers.'
---
...
Before scaffolding or modifying any component, always read and apply:
- **pix-styleguides CSS rules**: [../pix-styleguides/references/css.md](../pix-styleguides/references/css.md) - layers, nesting, custom properties, selector strategy.
- **pix-styleguides JS rules**: [../pix-styleguides/references/javascript.md](../pix-styleguides/references/javascript.md) - code splitting, lifecycle safety, const/let, early returns.
- **pix-styleguides HTML rules**: [../pix-styleguides/references/html.md](../pix-styleguides/references/html.md) - semantic elements, heading hierarchy, accessibility basics.
- **pix-design-system tokens**: reference `../pix-design-system/references/FOUNDATIONS.md` for all visual token decisions (color, spacing, radii, elevation).

These rules are non-negotiable and override any generic convention.
...
node ./.agents/skills/pix-custom-element/scripts/scaffold-component.mjs \
  --name "PixCard" \
  --target "<project-root>"
...
```bash
node --test ./.agents/skills/pix-custom-element/scripts/scaffold-component.test.mjs
```
...
2. **Resolve tag** - `static isAttribute` wins; otherwise the class name is converted from PascalCase to kebab-case (e.g. `PixDetails` becomes `pix-details`).
